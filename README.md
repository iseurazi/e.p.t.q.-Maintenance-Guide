...
이 프로젝트는 일반 HTML 파일을 직접 수정하는 사이트가 아닙니다. `src/` 안의 원본 파일을 수정한 뒤 `npm run build`를 실행하면, 업로드용 정적 파일이 `cafe24/` 폴더에 만들어집니다.
...
5. `npm run build`를 실행해 배포 파일을 다시 만듭니다.
6. `cafe24/` 폴더 안의 전체 내용을 FTP로 업로드합니다.
...
`npm run build`가 성공하면 `cafe24/` 폴더가 최신 업로드본으로 갱신됩니다.
...
| 홈 브랜드 스토리 문구 | `src/components/home-brand-section.tsx` |
| 홈 컬렉션 슬라이드 문구와 스펙 | `src/components/home-collection-section.tsx` |
...
| 컬렉션 상세 비포애프터 사례 | `src/components/collection-before-after-data.tsx` |
...
| 홈 브랜드 | `public/home-brand.webp` | `public/home-brand-mobile.webp` |
| 홈 익스피리언스 | `public/home-experience.webp` | `public/home-experience-mobile.webp` |
...
| Clinical Evidence 상단 | `public/clinical-evidence-visual.webp` | `public/clinical-evidence-visual-mobile.webp` |
| Before & After 상단 | `public/before-after-visual.webp` | `public/before-after-visual-mobile.webp` |
...
홈 페이지의 섹션 순서는 `src/app/page.tsx` 아래쪽에서 확인할 수 있습니다.
...
섹션 순서를 바꾸면 스크롤 애니메이션 영향이 있을 수 있으므로, 단순 문구 수정만 필요한 경우 이 순서는 건드리지 마세요.
...
파일: `src/components/home-brand-section.tsx`

제목은 `brandTitle`, 본문은 `brandCopyLines`를 수정합니다.

```tsx
const brandTitle = { ENG: "BRAND STORY", CHN: "品牌故事" };

const brandCopyLines = {
  ENG: [
    "첫 번째 줄",
    "두 번째 줄",
    "세 번째 줄",
...
배열 한 칸이 한 줄에 가깝게 쓰입니다. 줄 수를 바꾸면 레이아웃이 달라질 수 있으므로 되도록 기존 3줄 구조를 유지하세요.

### 홈 컬렉션 슬라이드

파일: `src/components/home-collection-section.tsx`

`collectionSlides` 배열 안에 S100, S300, S500 데이터가 들어 있습니다.

자주 수정하는 항목은 아래와 같습니다.
...
```tsx
{
  ...
  specs: [
    {
      term: { ENG: "Target Indication", CHN: "适用部位" },
      description: { ENG: "Lips...", CHN: "唇部..." },
    },
  ],
}
```

`description`은 PC, `mobileDescription`은 모바일에서 보이는 문구입니다. 둘 다 확인해야 합니다.

제품 상세 페이지의 내용과 홈 컬렉션 슬라이드 내용이 서로 다를 수 있습니다. 제품 스펙을 바꿀 때는 홈 슬라이드와 상세 페이지를 같이 확인하세요.
...
{
  href: "/collections/s100",
  label: { ENG: "COLEECTIONS", CHN: "产品系列" },
  children: [
...
파일: `src/components/collection-before-after-data.tsx`

S100은 `s100BeforeAfterItems`, S300은 `s300BeforeAfterItems`, S500은 `s500BeforeAfterItems`를 수정합니다.

```tsx
{
  title: <Localized value={{ ENG: "Forehead", CHN: "额头" }} />,
  beforeImage: {
    src: "/collections-s100-forehead-before.webp",
    alt: "Forehead before using S100",
    width: 491,
    height: 315,
  },
  afterImage: {
    src: "/collections-s100-forehead-after.webp",
    alt: "Forehead after using S100",
    width: 491,
    height: 315,
  },
  afterLabel: <Localized value={{ ENG: "Post 3 Months", CHN: "3个月后" }} />,
  beforeMeta: "Model:e.p.t.q.® S 100 lidocaine",
  afterMeta: "Volume:3.0ml",
}
```

이미지만 바꾸는 경우 기존 파일명을 유지해 `public/`의 이미지만 교체하는 방식이 가장 안전합니다.

사례를 추가하려면 같은 형식의 블록을 배열 안에 하나 더 추가합니다. 각 블록 사이에는 쉼표가 필요합니다.
...
5. `npm run build`를 실행해 `cafe24/contact-submit.php`에 반영합니다.
6. `cafe24/` 폴더 전체를 FTP 업로드합니다.
7. 운영 도메인의 Contact Us에서 실제 문의를 제출해 수신함, 스팸함, Reply-To 동작을 확인합니다.
...
| `src/components/home-process-section.tsx` | GSAP, ScrollTrigger, Swiper가 함께 쓰입니다. |
| `src/components/home-collection-section.tsx` | PC 스크롤 고정과 모바일 슬라이더 동작이 함께 들어 있습니다. |
| `src/components/home-before-after-section.tsx` | PC 스크롤 고정과 모바일 슬라이더 동작이 함께 들어 있습니다. |
...
| 브랜드 스토리 문구 바꾸기 | `src/components/home-brand-section.tsx` |
...
| 비포애프터 사례 바꾸기 | `src/components/collection-before-after-data.tsx` |
